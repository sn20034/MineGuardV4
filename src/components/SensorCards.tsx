...
function TrendArrow({ current, previous }: { current: number; previous: number | undefined }) {
  if (previous === undefined) return <Minus className="w-3.5 h-3.5 text-slate-600" />;
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      {cards.map((card) => {
        ...
        const prevVal = prev ? (card.label === "Tilt" ? prev.tilt : card.label === "Vibration" ? prev.vibration : card.label === "Displacement" ? prev.displacement : prev.crack) : undefined;
        return (
          <div
            key={card.label}
            className="relative rounded-xl border border-slate-800/60 bg-slate-900/40 p-4 hover:border-slate-700/60 transition-all duration-200 group"
          >
            <div className="flex items-start justify-between mb-3">
              ...
              {card.simulated && (
                <span
                  className="text-[9px] font-semibold uppercase tracking-wider text-slate-600 border border-slate-700/50 rounded px-1.5 py-0.5 cursor-help"
            ...
            <div className="mt-2 h-1 rounded-full bg-slate-800/60 overflow-hidden">
              <div
                ...
                style={{
                  width: `${Math.min(100, card.label === "Crack Width" ? (card.value > 0 ? 100 : 0) : (card.value / (card.label === "Tilt" ? 2.5 : card.label === "Vibration" ? 50 : 5)) * 100)}%`,
                }}
              />
            </div>
          </div>
